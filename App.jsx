...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const COLORS = {
  crypto: '#f59e0b', tunai: '#10b981', bank: '#3b82f6',
  ewallet: '#ef4444', saham: '#8b5cf6', hutang: '#f43f5e', talangan: '#fb923c'
}
const KAT_ICON = {
  'Makan & Minum': '🍽', Transportasi: '⛽', Perlengkapan: '👕',
  Sosial: '🤝', Komunikasi: '📶', GoFood: '🍱', GoRide: '🛵',
  Gaji: '💼', 'Lain-lain': '📌', Kesehatan: '💊', Hiburan: '🎮',
  Tagihan: '🧾', Sedekah: '🕌', Bensin: '⛽'
}
const KAT_COLOR = {
  'Makan & Minum': '#f97316', Transportasi: '#6366f1', Perlengkapan: '#3b82f6',
  Sosial: '#ec4899', Komunikasi: '#14b8a6', GoFood: '#ef4444', GoRide: '#10b981',
  Gaji: '#10b981', 'Lain-lain': '#94a3b8', Kesehatan: '#f43f5e',
  Hiburan: '#8b5cf6', Tagihan: '#f59e0b', Sedekah: '#10b981', Bensin: '#6366f1'
}
...
const INIT_TRX = [
  { id: uid(), tanggal: '2026-04-03', jam: '14:30', ket: 'Belanja Polo, Sarung Tangan, & Buff', kategori: 'Perlengkapan', jenis: 'keluar', nominal: 100000, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-03', jam: '16:00', ket: 'Bowo / Kondangan', kategori: 'Sosial', jenis: 'keluar', nominal: 50000, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-03', jam: '19:15', ket: 'Pinjaman Kakak', kategori: 'Lain-lain', jenis: 'keluar', nominal: 41000, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-03', jam: '21:55', ket: 'Paket Data Internet', kategori: 'Komunikasi', jenis: 'keluar', nominal: 25000, metode: 'BCA Xpresi' },
  { id: uid(), tanggal: '2026-04-04', jam: '13:45', ket: 'Makan Siang (Manis Ae)', kategori: 'Makan & Minum', jenis: 'keluar', nominal: 240790, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-04', jam: '17:15', ket: 'Bensin (Full Tank)', kategori: 'Transportasi', jenis: 'keluar', nominal: 89000, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-04', jam: '17:25', ket: 'Sedekah', kategori: 'Sosial', jenis: 'keluar', nominal: 2000, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-04', jam: '17:30', ket: 'Minum (Aqua)', kategori: 'Makan & Minum', jenis: 'keluar', nominal: 8000, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-04', jam: '17:45', ket: 'Jajan (Seblak)', kategori: 'Makan & Minum', jenis: 'keluar', nominal: 45000, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-05', jam: '14:15', ket: 'Makan Siang (Ayam Nelongso)', kategori: 'Makan & Minum', jenis: 'keluar', nominal: 58000, metode: 'Tunai' },
  { id: uid(), tanggal: '2026-04-05', jam: '15:30', ket: 'Parkir Nelongso', kategori: 'Transportasi', jenis: 'keluar', nominal: 5000, metode: 'Tunai' },
]

const INIT_GOALS = [
  { id: uid(), nama: 'Pajak Motor', tipe: 'tagihan', target: 200000, terkumpul: 0, deadline: '2026-12-31', frekuensi: 'tahunan', catatan: 'Bayar STNK tahunan', tanggalRutin: 0, rekening: '' },
  { id: uid(), nama: 'Admin BCA', tipe: 'tagihan_rutin', target: 15000, terkumpul: 0, deadline: '', frekuensi: 'bulanan', tanggalRutin: 16, rekening: 'BCA Xpresi', catatan: 'Biaya admin bulanan' },
]

// ─── UI PRIMITIVES ────────────────────────────────────────────────────────────
function Card({ children, style = {} }) {
  return (
    <div style={{ background: '#1a1f2e', borderRadius: 14, padding: '14px 16px', border: '1px solid #252d40', marginBottom: 10, ...style }}>
      {children}
    </div>
  )
}

function Pill({ label, color = '#64748b' }) {
  return (
...
function Btn({ children, onClick, color = '#3b82f6', full, small, style = {} }) {
  return (
    <button onClick={onClick} style={{ background: color + '1a', color, border: `1px solid ${color}44`, borderRadius: 9, padding: small ? '5px 11px' : '10px 16px', fontSize: small ? 11 : 13, fontWeight: 700, cursor: 'pointer', width: full ? '100%' : undefined, ...style }}>
      {children}
    </button>
  )
}
...
function NumInp({ label, value, onChange, placeholder = '0' }) {
  ...
  return (
    <div style={{ marginBottom: 9 }}>
      ...
      <div style={{ position: 'relative' }}>
        <span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 12, color: '#64748b', pointerEvents: 'none' }}>Rp</span>
...
function Inp({ label, style: inpStyle = {}, ...p }) {
  return (
    <div style={{ marginBottom: 9 }}>
      {label && <div style={{ fontSize: 11, color: '#64748b', marginBottom: 3, fontWeight: 600 }}>{label}</div>}
      <input {...p} style={{ width: '100%', background: '#10131e', border: '1px solid #252d40', borderRadius: 8, padding: '9px 11px', color: '#e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box', ...inpStyle }} />
    </div>
  )
}
...
function Sel({ label, children, ...p }) {
  return (
    <div style={{ marginBottom: 9 }}>
      {label && <div style={{ fontSize: 11, color: '#64748b', marginBottom: 3, fontWeight: 600 }}>{label}</div>}
      <select {...p} style={{ width: '100%', background: '#10131e', border: '1px solid #252d40', borderRadius: 8, padding: '9px 11px', color: '#e2e8f0', fontSize: 13, outline: 'none' }}>
        {children}
      </select>
    </div>
  )
...
// ─── BAR CHART ────────────────────────────────────────────────────────────────
function BarChart({ data, height = 120, color = '#3b82f6' }) {
      ...
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, height: '100%' }}>
          <div style={{ flex: 1, width: '100%', display: 'flex', alignItems: 'flex-end' }}>
            <div style={{ width: '100%', background: color + 'cc', borderRadius: '3px 3px 0 0', height: `${(d.v / max) * 100}%`, minHeight: d.v > 0 ? 3 : 0 }} />
          </div>
...
// ─── GOFOOD / GORIDE FORM ─────────────────────────────────────────────────────
function GoForm({ type, onAdd, onClose }) {
  ...
  const handleSimpan = () => {
    if (!tagihN) return
    onAdd({
      id: uid(), tanggal: todayStr(), jam: nowTime(), jenis: 'masuk',
      kategori: type, sumber: 'Gojek',
      ket: ket || `${type} – ${metode}`,
      nominal: actualBersih, metode,
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000c', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div style={{ background: '#1a1f2e', borderRadius: '16px 16px 0 0', padding: 20, width: '100%', maxWidth: 430, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <NumInp label="Total Tagih ke Customer" value={tagih} onChange={setTagih} />
        {metode === 'Tunai' && (
          <>
            <NumInp label="Aktual Dibayar Customer" value={aktual} onChange={setAktual} placeholder={tagih || '0'} />
        ...
        <NumInp label="💰 Uang Tambahan / Tip Ekstra (opsional)" value={uangTambahan} onChange={setUangTambahan} placeholder="0" />
        {tagihN > 0 && (
          <div style={{ background: '#10131e', borderRadius: 10, padding: 12, marginBottom: 14, fontSize: 12, lineHeight: 1.9 }}>
            ...
            <div style={{ borderTop: '1px solid #252d40', marginTop: 6, paddingTop: 6, display: 'flex', justifyContent: 'space-between', fontWeight: 800 }}>
              <span style={{ color: '#10b981' }}>Pendapatan Bersih</span>
              <span style={{ color: '#10b981', fontSize: 14 }}>Rp {fmtNum(actualBersih)}</span>
            </div>
          </div>
        )}
        <Btn full color="#10b981" onClick={handleSimpan}>✓ Simpan Pendapatan {type}</Btn>
      </div>
    </div>
  )
}
...
// ─── TRX FORM ────────────────────────────────────────────────────────────────
function TrxForm({ onAdd, onClose, edit }) {
      ...
      <div style={{ background: '#1a1f2e', borderRadius: '16px 16px 0 0', padding: 20, width: '100%', maxWidth: 430, maxHeight: '92vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
          {['keluar', 'masuk', 'mutasi'].map((j) => (
            <Btn key={j} small color={jenis === j ? jColor[j] : '#475569'} style={{ flex: 1, textAlign: 'center' }}
              onClick={() => { setJenis(j); setKat(cats[j][0]) }}>
        ...
        <NumInp label="Nominal" value={nominal} onChange={setNominal} />
        ...
        <Btn full color={jColor[jenis]} onClick={handleSimpan}>✓ Simpan</Btn>
      </div>
    </div>
  )
}
...
// ─── GOAL FORM ───────────────────────────────────────────────────────────────
function GoalForm({ onAdd, onClose, edit }) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#000c', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div style={{ background: '#1a1f2e', borderRadius: '16px 16px 0 0', padding: 20, width: '100%', maxWidth: 430, maxHeight: '92vh', overflowY: 'auto' }}>
        ...
        <Sel label="Tipe" value={tipe} onChange={(e) => setTipe(e.target.value)}>
          ...
          <option value="tagihan">🧾 Tagihan (sekali/tahunan)</option>
        ...
        <NumInp label="Nominal Target (Rp)" value={target} onChange={setTarget} />
        <NumInp label="Sudah Terkumpul (Rp)" value={terkumpul} onChange={setTerkumpul} />
        {tipe === 'tagihan_rutin' && (
          <>
            <Inp label="Tanggal Jatuh Tempo Tiap Bulan" type="number" value={tanggalRutin} onChange={(e) => setTanggalRutin(e.target.value)} placeholder="Contoh: 16" />
        ...
        <Btn full color="#8b5cf6" onClick={handleSimpan}>✓ Simpan Goal</Btn>
      </div>
    </div>
  )
}
...
// ─── ASET FORM ───────────────────────────────────────────────────────────────
function AsetForm({ onSave, onClose, edit }) {
  const [nama, setNama] = useState(edit?.nama || '')
  ...
  const handleKategoriChange = (e) => {
    ...
    setIcon(iconMap[e.target.value] || '💰')
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000c', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div style={{ background: '#1a1f2e', borderRadius: '16px 16px 0 0', padding: 20, width: '100%', maxWidth: 430, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <Sel label="Kategori Aset" value={kategori} onChange={handleKategoriChange}>
        ...
        <NumInp label={kategori === 'hutang' ? 'Jumlah Hutang (Rp)' : 'Saldo / Nilai (Rp)'} value={nilai} onChange={setNilai} />
        <Inp label="Keterangan" value={sub} onChange={(e) => setSub(e.target.value)} placeholder="Contoh: No. rek 123456, 100 lot saham" />
        <Inp label="Icon (emoji)" value={icon} onChange={(e) => setIcon(e.target.value)} inpStyle={{ width: 80, fontSize: 22, textAlign: 'center' }} />
        <Btn full color="#3b82f6" onClick={() => { if (!nama.trim()) return; onSave({ id: edit?.id || uid(), nama, kategori, nilai: parseDotNum(nilai), sub, icon }); onClose() }}>
          ✓ Simpan Aset
        </Btn>
      </div>
    </div>
...
// ─── AI CHAT ─────────────────────────────────────────────────────────────────
function AIChat({ aset, trx, goals, onClose }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 1000, system: sys, messages: [...histMsgs, { role: 'user', content: q }] }),
      })
      const data = await res.json()
      setMsgs((m) => [...m, { r: 'a', t: data.content?.map((c) => c.text || '').join('') || 'Maaf, coba lagi.' }])
    } catch {
      setMsgs((m) => [...m, { r: 'a', t: 'Koneksi bermasalah. Coba lagi.' }])
    }
    setLoading(false)
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#10131e', zIndex: 300, display: 'flex', flexDirection: 'column' }}>
      <div style={{ background: '#1a1f2e', borderBottom: '1px solid #252d40', padding: '13px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ color: '#e2e8f0', fontWeight: 800, fontSize: 15 }}>🤖 AI Asisten Keuangan</div>
          <div style={{ color: '#64748b', fontSize: 11 }}>Powered by Claude AI</div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 13px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {msgs.map((m, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: m.r === 'u' ? 'flex-end' : 'flex-start' }}>
            <div style={{ maxWidth: '85%', padding: '10px 13px', borderRadius: m.r === 'u' ? '14px 14px 4px 14px' : '14px 14px 14px 4px', background: m.r === 'u' ? '#2563eb' : '#1e2a3a', color: '#e2e8f0', fontSize: 13, lineHeight: 1.65, whiteSpace: 'pre-wrap' }}>
              {m.t}
            </div>
          </div>
        ))}
        {loading && (
          <div style={{ display: 'flex' }}>
            <div style={{ padding: '10px 14px', borderRadius: '14px 14px 14px 4px', background: '#1e2a3a', color: '#64748b', fontSize: 13 }}>⏳ Menganalisis...</div>
          </div>
        )}
        <div ref={endRef} />
      </div>
      <div style={{ padding: '10px 12px', background: '#1a1f2e', borderTop: '1px solid #252d40', display: 'flex', gap: 8 }}>
        <input value={inp} onChange={(e) => setInp(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && send()} placeholder="Tanya tentang keuanganmu..."
          style={{ flex: 1, background: '#10131e', border: '1px solid #252d40', borderRadius: 10, padding: '10px 13px', color: '#e2e8f0', fontSize: 13, outline: 'none' }} />
...
// ─── REKAP HARIAN ────────────────────────────────────────────────────────────
function RekapHarian({ trx, onClose }) {
  const grouped = {}
        ...
        {days.map((tgl) => {
          ...
          return (
            <Card key={tgl} style={{ marginBottom: 14 }}>
              <div style={{ fontWeight: 800, color: '#e2e8f0', marginBottom: 8, fontSize: 13 }}>{tglLabel}</div>
              <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
                {[['MASUK', masuk, '#10b981'], ['KELUAR', keluar, '#ef4444'], ['NET', net, net >= 0 ? '#10b981' : '#ef4444']].map(([lbl, val, c]) => (
                  <div key={lbl} style={{ flex: 1, background: c + '18', border: `1px solid ${c}30`, borderRadius: 8, padding: '6px 8px' }}>
                    <div style={{ fontSize: 9, color: c }}>{lbl}</div>
                    <div style={{ fontSize: 12, fontWeight: 800, color: c }}>{lbl === 'NET' && net >= 0 ? '+' : ''}{fmt(val)}</div>
                  </div>
                ))}
              </div>
...
export default function App() {
  ...
  const saveAset = (a) => setAset((prev) => { const e = prev.find((x) => x.id === a.id); return e ? prev.map((x) => x.id === a.id ? a : x) : [...prev, a] })
  const delAset = (id) => setAset((prev) => prev.filter((x) => x.id !== id))
  ...
  const calcGoal = (g) => {
    ...
    const deadlineD = new Date(g.deadline)
    const hariSisa = Math.max(0, Math.floor((deadlineD - todayD) / 86400000))
  ...
  const todayD = new Date()
  const rutinAlerts = goals
    .filter((g) => g.tipe === 'tagihan_rutin' && g.tanggalRutin)
    .map((g) => {
      const tglHariIni = todayD.getDate()
    ...
    <div style={{ minHeight: '100vh', background: '#10131e', color: '#e2e8f0', fontFamily: 'system-ui, sans-serif', maxWidth: 430, margin: '0 auto', position: 'relative', fontSize: 14 }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 90, background: '#1a1f2e', borderBottom: '1px solid #252d40', padding: '12px 16px 10px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 5, paddingTop: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
        ...
        <div style={{ display: 'flex', gap: 5 }}>
          {[['⬇', fmt(totalMasuk), '#10b981', 'MASUK'], ['⬆', fmt(totalKeluar), '#ef4444', 'KELUAR'], ['≈', (net >= 0 ? '+' : '') + fmt(net), net >= 0 ? '#10b981' : '#ef4444', 'NET'], ['🛵', fmt(totalGojek), '#f59e0b', 'GOJEK']].map(([icon, val, c, label]) => (
            <div key={label} style={{ flex: 1, background: c + '15', border: `1px solid ${c}30`, borderRadius: 8, padding: '5px 6px' }}>
              <div style={{ fontSize: 9, color: c, fontWeight: 700 }}>{icon} {label}</div>
              <div style={{ fontSize: 10, fontWeight: 800, color: c }}>{val}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '12px 14px 80px' }}>
        ...
        {tab === 'trx' && (
          <>
            <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
              {['semua', 'masuk', 'keluar', 'mutasi'].map((j) => (
                <Btn key={j} small color={filterJ === j ? (j === 'masuk' ? '#10b981' : j === 'keluar' ? '#ef4444' : j === 'mutasi' ? '#8b5cf6' : '#3b82f6') : '#475569'} onClick={() => setFilterJ(j)}>
                  {j === 'semua' ? 'Semua' : j === 'masuk' ? '⬇ Masuk' : j === 'keluar' ? '⬆ Keluar' : '↔ Mutasi'}
                </Btn>
              ))}
              ...
              <Card key={t.id} style={{ padding: '11px 13px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0, paddingLeft: 8 }}>
                    <div style={{ fontSize: 14, fontWeight: 900, color: jColor[t.jenis], marginBottom: 5 }}>
                      {t.jenis === 'masuk' ? '+' : t.jenis === 'keluar' ? '−' : '↔'}Rp {fmtNum(t.nominal)}
                    </div>
                    <div style={{ display: 'flex', gap: 5 }}>
        ...
        {tab === 'aset' && (
          <>
            ...
            <Btn full color="#3b82f6" style={{ marginBottom: 10 }} onClick={() => { setEditAsetItem(null); setModal('editAset') }}>
              ➕ Tambah Aset / Rekening / Saham / Hutang
            </Btn>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          ...
                          <div style={{ display: 'flex', gap: 4 }}>
                            ...
                            <Btn small color="#ef4444" onClick={() => delAset(a.id)}>🗑</Btn>
                          </div>
                        </div>
                      </div>
                      <div style={{ marginTop: 8 }}><MiniBar value={a.nilai} max={totalAset} color={COLORS[a.kategori] || '#64748b'} /></div>
                      {a.id === 'btc' && (
                        <div style={{ marginTop: 8, background: '#10131e', borderRadius: 7, padding: 9, fontSize: 11, color: '#94a3b8', lineHeight: 1.7 }}>
          ...
          <>
            ...
            <Card>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, fontSize: 11 }}>
                ...
                <span style={{ color: '#10b981', fontWeight: 700 }}>{fmt(totalMasuk)}</span>
              </div>
            </Card>
            ...
            <Card>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, fontSize: 11 }}>
                <span style={{ color: '#64748b' }}>Total</span>
                <span style={{ color: '#ef4444', fontWeight: 700 }}>{fmt(totalKeluar)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11 }}>
                <span style={{ color: '#64748b' }}>Rata/hari</span>
                <span style={{ color: '#ef4444', fontWeight: 700 }}>{fmt(rataKeluar)}</span>
              </div>
            </Card>
        ...
        {tab === 'goals' && (
          <>
            <Btn full color="#8b5cf6" style={{ marginBottom: 10 }} onClick={() => { setEditGoalItem(null); setModal('goal') }}>
              ➕ Tambah Goal / Tagihan / Target
            </Btn>
                ...
                <Card key={g.id}>
                  ...
                  {calc && (
                    <div style={{ background: '#10131e', borderRadius: 8, padding: 10, fontSize: 11, lineHeight: 1.8 }}>
                      ...
                      {hariCount < 7 && <div style={{ color: '#f59e0b', fontSize: 10, marginTop: 4 }}>* Data masih {hariCount} hari. Estimasi akurat setelah 7+ hari.</div>}
                    </div>
                  )}
      ...
      {modal === 'editAset' && <AsetForm onSave={saveAset} onClose={() => { setModal(null); setEditAsetItem(null) }} edit={editAsetItem} />}
